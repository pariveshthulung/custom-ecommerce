...
public class AddProductItemCommandHandler
    : ICommandHandler<AddProductItemCommand, BaseResult<Guid>>
{
    ...
    public async Task<BaseResult<Guid>> Handle(AddProductItemCommand request, CancellationToken cancellationToken)
    {
        var product = await _productRepository.GetByGuidAsync(request.ProductGuid, cancellationToken);
        var productItem = ProductItem.Create(request.Image, request.Sku, request.Quantity, request.Price);
